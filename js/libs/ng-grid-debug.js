...
(function (window, $) {
    ...
    domUtilityService.BuildStyles = function ($scope, grid, digest) {
      ...
      for (var i = 0; i < cols.length; i++) {
        ...
        if (col.visible !== false) {
          ...
          css += "." + gridId + " .col" + i + " { width: " + (col.width + rightPad) + "px; left: " + sumWidth + "px; height: " + rowHeight + "px }" + "." + gridId + " .colt" + i + " { width: " + (col.width + rightPad) + "px; }";
          sumWidth += col.width;
        }
      }
  ...
  angular.module('ngGrid.services').factory('$sortService', ['$parse', '$utilityService', function ($parse, $utils) {
    ...
    sortService.sortData = function (sortInfo, data /*datasource*/) {
      ...
      var l = sortInfo.fields.length, order = sortInfo.fields, col, direction, // IE9 HACK.... omg, I can't reference data array within the sort fn below. has to be a separate reference....!!!!
          d = data.slice(0);
    ...
    self.configureColumnWidths = function () {
      ...
      angular.forEach($scope.columns, function (ngCol, i) {
        ...
        if (!$utils.isNullOrUndefined(ngCol.originalIndex)) {
          ...
          if (ngCol.isAggCol && ngCol.visible) { // aggregate columns are 25px in length. 
            totalWidth += 25;
          }
      });
