...
controlerApp.controller('ControlerCtrl', function ($rootScope, $scope, $http, $location, $timeout) {
  ...
  $scope.salvar = function (entity) {
    if (validaTabela(entity, $scope)) {
      $http.post('/horarios/salvar', { entity : entity}).success(function (res) {
  ...
  $scope.apagar = function (entity, rowid) {
    if (entity.id === undefined) {
      ...
      if (r == true) {
        $http.post('/horarios/apagar', { entity : entity}).success(function (res) {
  ...
  $scope.changeDate = function () {
    ...
    var user = $rootScope.globals.currentUser;
    $http.post("/horarios/fechamento/mes", {user : user, month : month, year : year}).success(function (response) {
  ...
  $scope.loadGrid = function () {
    ...
    var user = $rootScope.globals.currentUser;
    $scope.myData = null;
    $http.post("/horarios/list", {user : user, month : month, year : year}).success(function (response) {
      $scope.myData = response;
    })
  }
  function validaTabela(entity, $scope) {
    var resposta = true;
    if (entity.data === undefined || entity.hora_entrada === undefined || entity.hora_saida === undefined || entity.atividade === undefined || entity.observacao === undefined) {
