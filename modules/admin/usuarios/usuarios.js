...
usuariosApp.controller('UsuariosCtrl', function ($rootScope, $scope, $http, $location, $modal, $document, $timeout) {
  ...
  $scope.apagar = function (entity) {
    if (entity.ativo == 0) {
      ...
      if (r == true) {
        $http.post('/usuarios/apagar', { entity : entity}).success(function (res) {
  ...
  function loadGrid() {
    $http.post('/usuarios/list').success(function (res) {
      $scope.usuarios = res;
    });
  }
...
usuariosApp.controller('ModalUsuariosMostrarCtrl', function ($scope, $http, $modalInstance, $filter, usuario) {
  $scope.loadDadosPessoais = function () {
    $http.post('/usuarios/dados/list', {usuario : usuario}).success(function (res) {
      $scope.usuario = res;
    });
  };
  ...
  $scope.salvar = function () {
    ...
    $http.post('/usuarios/dados/salvar', {usuario : usuario}).success(function (res) {
      $modalInstance.close(res);
    });
  };
  $scope.loadTipoContratacao = function () {
    $http.post('/usuarios/dados/contratacao/list').success(function (res) {
...
usuariosApp.controller('ModalUsuariosEditarCtrl', function ($scope, $http, md5, $modalInstance, usuario) {
  $scope.save = function () {
    ...
    if (usuario.senha !== usuario.senha2) {
      ...
      if (usuario.id_usuario === undefined && (usuario.senha2 === undefined || usuario.senha === undefined)) {
        ...
        $http.post('/usuarios/salvar', {usuario : usuario}).success(function (res) {
          console.log(res);
  ...
  $scope.loadPerfil = function () {
    $http.post('/usuarios/perfil/list').success(function (res) {
      $scope.perfis = res;
    });
  };
