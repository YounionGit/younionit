...
test('elements and indexes', function (t) {
  ...
  t.deepEqual(map([x, y, z], function (c, i) {
    return i
  }), [ 0, 1, 2 ], 'index check');
  t.deepEqual(map([x, y, z], function (c, i) {
    return i
  }), [ 0, 1, 2 ], 'crippled index check');
  var xs0 = [ x, y, z ];
...
test('ignore holes', function (t) {
  t.plan(2);
  t.deepEqual(map(new Array(5), function (x) {
    return x
  }), []);
  t.deepEqual(map(cripple(new Array(5)), function (x) {
    return x
  }), []);
});
test('sparse map', function (t) {
  ...
  xs[2] = 'a';
  xs[4] = 'b';
  t.equal(map(xs, function (x, i) {
    return x + i
  }).join(''), 'a2b4');
  var ys = new Array(5);
  ys[2] = 'a';
  ys[4] = 'b';
  t.equal(map(cripple(xs), function (x, i) {
    return x + i
  }).join(''), 'a2b4');
  t.end();
});
