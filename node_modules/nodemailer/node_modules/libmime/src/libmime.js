...
var libmime = module.exports = {
  ...
  encodeWords      : function (data, mimeWordEncoding, maxLength, fromCharset) {
    ...
    maxLength = maxLength || 0;
    var decodedValue = libcharset.decode(libcharset.convert((data || ''), fromCharset)), encodedValue;
  ...
  foldLines        : function (str, lineLength, afterSpace) {
    ...
    while (pos < len) {
      ...
      if ((match = line.match(/^[^\n\r]*(\r?\n|\r)/))) {
        ...
        if ((match = line.match(/(\s+)[^\s]*$/)) && match[0].length - (afterSpace ? (match[1] || '').length : 0) < line.length) {
          line = line.substr(0, line.length - (match[0].length - (afterSpace ? (match[1] || '').length : 0)));
        } else
          if ((match = str.substr(pos + line.length).match(/^[^\s]+(\s*)/))) {
