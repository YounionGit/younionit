var Dicer = require('..');
var assert = require('assert'), fs = require('fs'), path = require('path'), inspect = require('util').inspect;
...
function next() {
  if (t === tests.length)
    return;
  var v = tests[t], fixtureBase = FIXTURES_ROOT + v.source, fd, n = 0, buffer = new Buffer(v.chsize), state = { done : false, parts : [], preamble : undefined };
  fd = fs.openSync(fixtureBase + '/original', 'r');
  var dicer = new Dicer(v.opts), error, partErrors = 0, finishes = 0;
  ...
  dicer.on('part', function (p) {
    ...
    if (v.dicerError)
      assert(error !== undefined, makeMsg(v.what, 'Expected error')); else
